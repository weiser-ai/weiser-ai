...
class BaseCheck():
    ...
    def time_grain_alias(self) -> str:
        if self.check and self.check.time_grain:
            return self.snake_case(f'{self.check.time_grain.sql} {self.check.time_grain.granularity}') 
    ...
    def append_result(self, 
        ...
        if self.check.group_by or self.check.time_grain:
            ...
            result['name'] = '_'.join((result['name'], 
                                      '_'.join(map(lambda pair: '_'.join(pair), zip(group_by_columns, map(str, value[:-1]))))))
        result.update({
            'check_id': self.generate_check_id(dataset, result['name']),
            'datasource': self.datasource,
            'dataset': dataset,
            'actual_value': value[-1] if self.check.group_by or self.check.time_grain else value,
            'success': success,
            'fail': not success,
            'run_id': self.run_id,
