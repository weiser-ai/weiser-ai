...
class CheckAnomaly(BaseCheck):
    ...
    def run(self, verbose: bool) -> List[Any]:
        ...
        for dataset in datasets:
            ...
            m_i = (0.6745 * (rows[0][2] - rows[0][1]) / (rows[0][0])) if int(rows[0][0]) != 0 else 0
            success = self.apply_condition(m_i)
            self.append_result(success, rows[0][2], results, dataset, datetime.now(), verbose)
    ...
    def get_query(self, table: str, verbose: bool) -> Select:
        return self.build_query(['mad(actual_value)', 'median(actual_value)', 'last(actual_value)'], table, verbose=verbose)
    
    def build_query(self, select_stmnt: List[Any], table:str, limit: int=1, verbose: bool=False) -> Select:
        q = Select().from_(
                Select().from_(table).select('actual_value')\
                    .where(f"check_id = '{self.check.check_id}'").order_by('run_time ASC').subquery()
            ).select(*select_stmnt).limit(1)
        if verbose:
