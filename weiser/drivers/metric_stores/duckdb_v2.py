...
class DuckDBMetricStoreV2:
        ...
        with Session(self.engine) as session:
            ...
            if self._has_s3_config():
                try:
                    ...
                    session.exec(text(f"SET s3_access_key_id = '{self.config.s3_access_key}'"))
                    session.exec(text(f"SET s3_secret_access_key = '{self.config.s3_secret_access_key}'"))
    ...
    def _create_tables_direct(self) -> None:
        ...
        with Session(self.engine) as session:
            session.exec(text("""
                CREATE TABLE IF NOT EXISTS metrics_duckdb (
    ...
    def _export_to_s3(self, run_id: str) -> None:
        ...
        try:
            with Session(self.engine) as session:
                ...
                session.exec(text(f"""
                    COPY (SELECT * FROM metrics_duckdb WHERE run_id='{run_id}') 
                    TO '{s3_path}' (FORMAT 'parquet')
                """))
                
                # Create merged export
                merged_path = f"s3://{self.config.s3_bucket}/tmp/merged_at_{run_id}.parquet"
                session.exec(text(f"""
    ...
    def get_metrics_for_check(
        ...
        with Session(self.engine) as session:
            statement = select(MetricRecordDuckDB).where(MetricRecordDuckDB.check_id == check_id)
    ...
    def get_metrics_for_run(
        ...
        with Session(self.engine) as session:
            statement = select(MetricRecordDuckDB).where(MetricRecordDuckDB.run_id == run_id)
